...
for(i in 1:10){
  ...
  pred_ch_r <- predict.glm(log_reg, testData[,-1], type = "response")
  err_lreg[i] <- sum(abs(round(pred_ch_r)-(as.numeric(testData$chart_random)-1)))/length(pred_ch_r)
